...
class AggregationService:
    ...
    def _build_payload(
        ...
        *,
        ...
        normalized_measurements = self._normalize_measurements(measurements_data)
    ...
    def _normalize_measurements(self, measurements_data: list[dict]) -> list[dict]:
            ...
            normalized.append(
                {
                    ...
                    "measurement_type": self.TYPE_ALIASES.get(str(item.get("measurement_type", "unknown")), "unknown"),
                    "source_page": int(item.get("source_page", 0)),
                    "source_column": item.get("source_column", "import"),
                    "row_index_on_page": int(item.get("row_index_on_page", idx)),
                }
            )
